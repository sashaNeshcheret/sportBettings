...
public class GameService {
    ...
    public GameService(SportEventCreator sportEventCreator, BetService betService, UserService userService, UserFactory userFactory, WagerRepository wagerRepository, BetIO betIO) {
    ...
    public void iterate() {
        ...
        List<FootballSportEvent> sportEvents = sportEventCreator.generateSportsEvent();
        ...
        List<Outcome> winOutcomes = betService.gameSimulation();
...
//        betService.createDisplayedBets();
        List<DisplayedBet> displayedBets = betService.showCurrentDisplayedBets();
        if(displayedBets.isEmpty()){
            betService.createDisplayedBets();
            displayedBets = betService.showCurrentDisplayedBets();
        }
        DisplayedBet bet = betService.receiveBet(displayedBets);
        ...
        composeWager(player, bet, betAmount);
        userService.changeBalance(player, betAmount);
        userService.showCurrentBalance(player);
    }

    private void composeWager(User player, DisplayedBet bet, BigDecimal betAmount) {
